...
import { getFirestore, doc, setDoc, updateDoc } from "firebase/firestore";// para almacenar los datos del usuario en Firebase
...
export default {
  ...
  async mounted() { // this runs when the page "EditBLog" is complete created 
    // here I can get some components HTML such us, button, inputs, etc
    // ref allows us to take a componente from de HTML that is with that ref wi can get the HTML element
  ...
  methods: {
    ...
    async updateBlog() {
      ...
      const dataBase = await doc(db, "blogPosts", this.routeID);
      ...
      if (this.blogTitle.lenght !== 0 && this.blogHTML.lenght !== 0) {
        ...
        this.loading = true;
        // we update just the title and the html components, that is we do not update the cover 
        await updateDoc(dataBase, {
          blogHTML: this.blogHTML,
          blogTitle: this.blogTitle,
        });
